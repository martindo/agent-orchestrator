...
> **⚠️ Test-verification caveat:** the audit environment is sandboxed off PyPI (SSL failures on install), so the "1451 passing" number was verified by *static analysis* (counting `def test_`, reading assertions), NOT by running the suite. Anyone doing this work locally should run `pytest tests` after `pip install -e .[dev]` to establish a real baseline first. Counts check out (1343 runtime `def test_` + 95 studio = ~1438, ~matches), but there is **no CI**, so nothing has been gating that number.
...
- [x] **1.1 (S)** `_artifact_store` dead on every startup (`state_dir` used before assignment → swallowed `UnboundLocalError`). ✅ 2026-07-22 — moved the `state_dir` definition + `mkdir` above the artifact-store init; artifact persistence now initializes. Syntax-verified (`py_compile`); not runtime-tested (no PyPI in audit env).
- [x] **1.2 (S)** Silent mock LLM fallback → now raises. ✅ 2026-07-22 — `LLMAdapter.call` raises `ConfigurationError` when no provider is registered instead of returning a fabricated `confidence:0.5` success. No test asserted the old fake behavior (grep-verified). Syntax-verified.
...
- [ ] **2.1 (M)** **Enforce authentication.** ~178 endpoints across 23 route files with **zero** `Depends`/`Security`/auth middleware/CORS. The JWT machinery exists (`middleware/shared_auth.py` `create_token`/`verify_token`) but nothing calls it; `/auth/me` returns hardcoded `anonymous` (`api/auth_routes.py:48`). Add a configurable auth dependency (default off for local, on for deployment — secure default per profile) that verifies the token on the API routes. Same for the Studio API (`studio/app.py` — no auth at all).
- [ ] **2.2 (S)** Hardcoded JWT secret `"platform-shared-secret-2026"` (`shared_auth.py:15`) — anyone with the source can forge admin tokens. Move to an env var / settings; fail closed if unset in enforced mode.
...
- [ ] **2.5 (M)** ~15 endpoints accept raw `body: dict` with no Pydantic schema (all 4 auth endpoints included) — no validation, no 422s. Add request models.
...
- [ ] **5.1 (S)** **No CI** — `.github/` does not exist. The "1451 passing" is gated by nothing. Add a GitHub Actions workflow: install `.[dev]`, run `pytest tests` + the studio suite.
...
| Date | Item(s) | Notes |
|------|---------|-------|
| 2026-07-22 | 1.1, 1.2, 1.4 (+1.3 partial) | First correctness wave on branch `audit/fixes`. Fixed the artifact-store use-before-assignment (dead on every start), made the silent mock-LLM fallback raise instead of fabricating, corrected stale placeholder comments. **Syntax-verified only** (`py_compile`) — the audit environment has no PyPI access to run `pytest`, so run the suite locally to confirm. |
